...
def compute_power(bit_rate_pos, xput):
    base_pwr = pwr_base_table[bit_rate_pos]
    xput_pwr = net_model_coeff * xput + net_model_intercept
    pwr_bitrate = base_pwr + xput_pwr * VIDEO_BIT_RATE[bit_rate_pos] / xput

    #print ['compute_power:', VIDEO_BIT_RATE[bit_rate_pos], xput, pwr_bitrate]
    return pwr_bitrate
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'w') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            real_power = compute_power(bit_rate, float(video_chunk_size) / 1024.0 / float(delay) * 8 * M_IN_K)
            if real_power > POWER_BUDGET:
                reward = MIN_REWARD
            else:
                # -- linear reward --
